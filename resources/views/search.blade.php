    ...
    <section class="mt-5">
        <div class="container">
            <div class="row justify-content-center text-center text-white">
                <div class="col">
                    <h2 class="font-weight-bolder">Hasil Pencarian</h2>
                </div> 
            </div>
            <div class="row justify-content-center">
                @if (!$search->isEmpty())
                @else
                    <p class="text-white text-center">Mohon maaf, Nomor yang anda masukkan tidak tersedia.</p>            
                @endif
                @foreach ($search as $data)
                <button type="button" data-id="{{ $data->id }}" class="btn btn-light content mr-2 mt-3 modal-nomor"  style="width: 17rem;">
                    <h5>{{ $data->detail_nomor}}</h5>
                    <span class="font-weight-light content text-primary">Rp. {{ $data->harga}}</span>
                    <span class="text-black-50">
                        @if ($data->operator == "simpati")
                            simPATI
                        @elseif ($data->operator == "as")
                            Kartu AS
                        @elseif ($data->operator == "halo")
                            Kartu Halo
                        @elseif ($data->operator == "three")
                            Three
                        @elseif ($data->operator == "smartfren")
                            Smartfren
                        @elseif ($data->operator == "axis")
                            AXIS
                        @elseif ($data->operator == "indosat")
                            Indosat Ooredoo
                        @elseif ($data->operator == "xl")
                            XL
                        @endif
                    </span> 
                </button>   
                @endforeach
                <button type="button" class="btn mt-4">{{ $search }}</button>
